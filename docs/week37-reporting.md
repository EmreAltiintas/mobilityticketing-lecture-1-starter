...
5. **Commit/rollback:** the `payments` insert and the trigger's upsert are
   ...
   `UPDATE`/`DELETE` triggers (cases 3 to 5).
6. **When each report becomes current:** the direct query and the
   function are current on the very next read, no action needed. The
   trigger-table updates inside the same transaction, so any reader after
   commit sees it. The materialized view becomes current only after
   someone runs `REFRESH MATERIALIZED VIEW daily_captured_revenue`, which
   is not triggered automatically by anything in this migration.
...
- **Consequence:** the table is wrong from the moment it is created, not
  only after a later correction, cases 1 to 6 measure *drift going forward*,
...
every 5 to 15 minutes), with the **direct query/function** kept available as
