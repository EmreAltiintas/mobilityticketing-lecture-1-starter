-- Step 1: confirm the starting point before touching anything.
\set ON_ERROR_STOP on
select current_database();

select id, product_code, price, currency from tickets order by id;

-- Every ticket must point to a product that exists.
...
do $$
begin
    if (select count(*) from tickets) < 3
        or (select count(distinct product_code) from tickets) < 2 then
